...
class WelcomeController < ApplicationController
  ...
  def index(content = CONTENT, highlights = HIGHLIGHTS)
    # create paragraphs by spliting content into array
    paragraphs = content.split(/\n\s*\n/).reject(&:blank?)
    # loop though the array of paragraphs
    items = []
    paragraphs.each do |paragraph|
      highlights.to_a.each do |highlight|
        # loop though the array of highlights
        items.push([{'comment' => highlight[:"comment"].html_safe}, {'paragraph' => html_tags(paragraph, highlight[:"start"], highlight[:"end"], 'b').html_safe}])
      end
    end
  ...
  def html_tags(string, start, finish, tag)    
    string.scan(/((.)\2*)/)
      .map(&:first)
      .insert(start, "<#{tag} style=color:#{color_hex()}>")
      .insert(finish, "<\/#{tag}>")
      .join
  end

  # def html_tags(string, start, finish, tag)    
  #   string.chars.chunk {|c| c}
  #     .map {|_,a| a.join}
  #     .insert(start, "<#{tag}>")
  #     .insert(finish, "<\/#{tag}>")
  #     .join
